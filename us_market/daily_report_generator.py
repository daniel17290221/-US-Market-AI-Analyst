...
class USDailyReportGenerator:
    ...
    def generate_ai_content(self, raw_data):
        ...
        [핵심 요구사항 - 반드시 준수!]
        1. **뻔한 소제목 금지**: '지정학적 리스크', '투자 전략' 같이 매일 반복되는 평범한 제목은 절대 사용하지 마세요. 
        2. **날카로운 타이틀**: '금리 인하 기대로 반등' 같은 단순 요약보다는 'AI 거품론을 잠재운 실적의 힘', '채권 금리 발작이 휩쓴 기술주' 등 오늘의 핵심 주제를 관통하는 창의적인 비유를 섞은 제목을 써주세요.
        3. **매크로 분석 연계**: {raw_data.get('macro', {}).get('key_takeaways', '시장 변화')} 자료를 바탕으로, 단순한 지수 나열이 아닌 현재 뉴스 헤드라인과 실제 지수의 움직임을 연결해서 설명하세요.
        4. **다양한 테마**: 기술주 위주면 반도체 공정/공급망 이야기, 거시 경제 위주면 고용/소비자 심리 이야기 등 매일 주요 테마를 바꿔서 보고서를 살아있게 만드세요.

        [JSON 출력 형식]
        1. catchy_title: 오늘의 시장을 한 문장으로 정의하는 강력한 헤드라인
        2. summary_title: '핵심 요약' 대신 사용할 창의적인 요약 섹션 제목 (예: "오늘의 마켓 엑스레이", "3줄 투자 체크포인트")
        3. core_summary: [리버럴하고 인사이트 있는 분석 내용 3문장 (JSON 리스트)]
        4. briefing_title: '증시 브리핑' 대신 사용할 역동적인 제목 (예: "숫자로 보는 월가", "지수별 온도차")
        5. sections: 최소 3개의 심층 섹션. 각 섹션은 다음을 포함:
           - emoji_tag: 섹션 주제를 상징하는 임팩트 있는 이모지
           - title: 매일 변화하는 창의적이고 도발적인 소제목
           - content: 전문적이면서도 투자 열기가 느껴지는 구어체 분석 (300자 이상 상세히)
        6. hashtags: 관련 해시태그 5개 이상
        7. market_mood_narrative: 오늘 장을 한마디로 정의하는 문장
        """
