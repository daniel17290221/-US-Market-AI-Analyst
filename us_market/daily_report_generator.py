...
class USDailyReportGenerator:
    ...
    def generate_ai_content(self, raw_data):
        ...
        [핵심 요구사항 - 반드시 준수!]
        1. **실시간 사건 중심**: '지정학적 리스크' 같은 추상적 단어 대신, {raw_data.get('macro', {}).get('key_takeaways', [])}에 언급된 '실제 사건(예: 이란 공격, 엔비디아 실적 등)'을 소제목과 본문에 직접 핵심 키워드로 사용하세요.
        2. **지표와 뉴스 결합**: WTI, 나스닥 등 시장 지표의 변동 원인을 데이터에 있는 뉴스 헤드라인과 반드시 연결해서 설명하세요. (예: "이란 공격 소식에 WTI +8% 폭등, 물가 우려 재점화")
        3. **도발적이고 창의적인 타이틀**: '오늘의 증시' 같은 제목은 절대 금지. '화약고 된 중동, 유가 폭격이 부른 증시 쇼크', '반격의 기술주: AI는 죽지 않았다' 등 뉴스 내용을 반영한 강력한 제목을 쓰세요.
        4. **차별화된 섹션**: 기술주, 지정학, 거시 경제 등 매일 데이터에서 가장 비중이 크고 중요한 3가지 테마를 선정하여 섹션을 구성하세요.

        [JSON 출력 형식]
        1. catchy_title: 오늘의 핵심 뉴스를 반영한 강력한 헤드라인
        ...
        5. sections: 최소 3개의 심층 섹션. 각 섹션은 다음을 포함:
           - emoji_tag: 섹션 주제에 맞는 긴박한 이모지
           - title: 뉴스 키워드가 포함된 구체적이고 도발적인 소제목
           - content: 전문적이면서 인과관계가 명확한 분석 (300자 이상)
